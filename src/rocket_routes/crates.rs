...
#[rocket::get("/crates")]
pub async fn get_crates(db: DbConn, _user: EditorUser) -> Result<Value, Custom<Value>> {
    db.run(|c| {
...
pub async fn view_crate(id: i32, db: DbConn, _user: EditorUser) -> Result<Value, Custom<Value>> {
    db.run(move |c| {
